...
void CL_CALLBACK kernel_queued(cl_event e, cl_int status, void* data)
{
	printf("%s", (char*)data);
}

void CL_CALLBACK kernel_submitted(cl_event e, cl_int status, void* data)
{
	printf("%s", (char*)data);
}
...
int main() 
{
   ...
   /* Read the buffer */
   err = clEnqueueReadBuffer(queue, data_buffer, CL_FALSE, 0, sizeof(data), &data, 0, NULL, &read_event);
   if(err < 0) {
   ...
   /* Set event handling routines */
	 char *kernel_submitted_msg = "The kernel submitted successfully.\n\0";
	 char *kernel_queued_msg = "The kernel queued successfully.\n\0";
	 char *kernel_complete_msg = "The kernel finished successfully.\n\0";
	 err = clSetEventCallback(kernel_event, CL_QUEUED, &kernel_queued, kernel_queued_msg);
	 if (err < 0) {
		 perror("Couldn't set callback for event");
		 //exit(1);
	 }
	 err = clSetEventCallback(kernel_event, CL_SUBMITTED, &kernel_submitted, kernel_submitted_msg);
	 if (err < 0) {
		 perror("Couldn't set callback for event");
		 //exit(1);
	 }
	 err = clSetEventCallback(kernel_event, CL_COMPLETE, &kernel_complete, kernel_complete_msg);
   if(err < 0) {
      perror("Couldn't set callback for event");
      //exit(1);   
   }
