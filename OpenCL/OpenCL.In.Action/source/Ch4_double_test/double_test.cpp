...
cl_program build_program(cl_context ctx, cl_device_id dev, const char* filename, char* options) 
{
   cl_program program;
   FILE *program_handle;
...
int main() 
{
   ...
   clGetDeviceInfo(device, CL_DEVICE_EXTENSIONS, NULL, NULL, &ext_size);
   ...
   b_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float), &b, NULL);
