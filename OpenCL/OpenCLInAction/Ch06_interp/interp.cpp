...
void read_image_data(const char* filename, png_bytep* input, png_bytep* output, size_t* w, size_t* h) 
{
   ...
   *input = static_cast<png_bytep>(malloc(*h * png_get_rowbytes(png_ptr, info_ptr)));
   *output = static_cast<png_bytep>(malloc(*h * png_get_rowbytes(png_ptr, info_ptr) * SCALE_FACTOR  * SCALE_FACTOR ));
   for(i=0; i<*h; i++) {
...
int main(int argc, char **argv) 
{
   ...
   input_image = clCreateImage2D(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, &png_format, width, height, 0, (void*)input_pixels, &err);
   if(err < 0) {
   ...
   output_image = clCreateImage2D(context, CL_MEM_WRITE_ONLY, &png_format, SCALE_FACTOR * width, SCALE_FACTOR * height, 0, NULL, &err);
   if(err < 0) {
      perror("Couldn't create the image object");
      exit(1);
   }; 
   ...
   err = clEnqueueReadImage(queue, output_image, CL_TRUE, origin, region, 0, 0, (void*)output_pixels, 0, NULL, NULL);
   if(err < 0) {
