...
void read_image_data(const char* filename, png_bytep* data, size_t* w, size_t* h) 
{
   ...
   /* Open input file */
   FILE *png_input;
   if((png_input = fopen(filename, "rb")) == nullptr) 
	 {
   ...
   png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, NULL, NULL);
	 if (png_ptr == nullptr) abort();

   info_ptr = png_create_info_struct(png_ptr);
	 if (info_ptr == nullptr) abort();
   ...
   *data = static_cast<png_bytep>(malloc(*h * png_get_rowbytes(png_ptr, info_ptr)));
   for(i=0; i<*h; i++) {
...
void write_image_data(const char* filename, png_bytep data, size_t w, size_t h) 
{
   ...
   png_set_IHDR(png_ptr, info_ptr, w, h, 32, PNG_COLOR_TYPE_GRAY, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_BASE, PNG_FILTER_TYPE_BASE);
...
int main(int argc, char **argv) 
{
   ...
   input_image = clCreateImage2D(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, &png_format, width, height, 0, (void*)pixels, &err);
   output_image = clCreateImage2D(context, CL_MEM_WRITE_ONLY, &png_format, width, height, 0, NULL, &err);
   if(err < 0) {
   ...
   err = clEnqueueReadImage(queue, output_image, CL_TRUE, origin, region, 0, 0, (void*)pixels, 0, NULL, NULL);
   if(err < 0) {
